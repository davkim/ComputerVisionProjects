...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	# patch_rows = patch_cols = patchL * 2 + 1
		...
		for c in range(ssd_cols):
			...
			for i in range(patch_rows):
				for j in range(patch_cols):
					# do this only if TODOMask is 0
					if TODOMask[i,j] == 0:
						# do 3 color channels separately
						# red
						diff = textureIm[r+i, c+j, 0] * 1.0 - TODOPatch[i, j, 0] * 1.0
						p_ssd += diff * diff
						# green
						diff = textureIm[r+i, c+j, 1] * 1.0 - TODOPatch[i, j, 1] * 1.0
						p_ssd += diff * diff
						# blue
						diff = textureIm[r+i, c+j, 2] * 1.0 - TODOPatch[i, j, 2] * 1.0
						p_ssd += diff * diff
			SSD[r, c] = p_ssd
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	...
	# starting points
	imHole_i = iPatchCenter - patchL
	imHole_j = jPatchCenter - patchL
	patch_i = iMatchCenter - patchL
	patch_j = jMatchCenter - patchL
	for i in range(patchSize):
		for j in range(patchSize):
			...
			#
			if TODOMask[i, j] == 1:
				imHole[imHole_i+i, imHole_j+j, :] = textureIm[patch_i+i, patch_j+j, :]
...
im = Image.open('boat.jpg').convert('RGB')
